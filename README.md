...
- `data/`: A directory containing the synthetic data used to reproduce the analysis (`synthetic_dataset.rds`). 
           As well as the matrices containing the applied prior knowledge for the creation of all ABN models (`prior_knowledge.xlxs`).
...
- `scripts/`: A directory containing the R scripts used for data analysis and figure generation:
  ...
  - `05_management_effect.R`: Script for performing the analysis on the effect of management strategy in two parts:
                             ...
                             B) ABN analysis to explore dependencies between variables related to management strategy, inlcuding MCMCabn for all 10 imputed data sets.
...
- For any questions or further clarifications regarding the code or analyses, please refer to the contact information provided in the original paper.
